...
const STATUS_ROWS: { key: keyof MasterStats; label: string; color: string }[] = [
  { key: 'completedCount', label: 'Завершено', color: 'bg-emerald-500' },
  { key: 'acceptedCount', label: 'В работе', color: 'bg-blue-500' },
  { key: 'pendingCount', label: 'Ожидают', color: 'bg-amber-500' },
  { key: 'cancelledCount', label: 'Отменено', color: 'bg-rose-500' },
];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <StatTile
          ...
          value={loading || !stats || stats.avgAcceptLatencyMinutes === null ? '—' : `${stats.avgAcceptLatencyMinutes} мин`}
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <h4 className="text-2xl font-extrabold text-slate-900 dark:text-white mt-0.5">
                ${loading || !stats ? '—' : Number(stats.totalEarnings).toLocaleString()}
                <span className="text-xs font-bold text-slate-400 ml-1">всего</span>
              </h4>
            </div>
              ...
              return stats.earningsByCategory.map((c) => (
                <div key={c.categoryId} className="space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="font-extrabold text-slate-900 dark:text-white">
                      ${c.total} <span className="text-slate-400 font-semibold">({c.completedCount})</span>
                    </span>
                  </div>
