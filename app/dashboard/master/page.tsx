    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white mt-1">Master Dashboard</h1>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { title: 'Monthly Revenue', value: '$4,850.00', growth: '+14.2%', icon: 'DollarSign', color: 'from-amber-500 to-orange-500' },
          { title: 'Jobs Completed', value: '38 Jobs', growth: '+8.1%', icon: 'CheckCircle2', color: 'from-emerald-500 to-teal-500' },
          { title: 'Customer Rating', value: '4.95 / 5', growth: 'Top 1%', icon: 'Star', color: 'from-yellow-500 to-amber-500' },
          { title: 'Response Time', value: '11 mins', growth: 'Fast', icon: 'Clock', color: 'from-blue-600 to-sky-500' },
        ].map((m, idx) => (
      ...
      <div className="bg-gradient-to-r from-amber-900/40 via-slate-900 to-slate-900 p-6 rounded-3xl border border-amber-800/40 shadow-xl space-y-3">
        <div className="flex justify-between items-center text-xs text-white">
          <span className="font-bold">Master Profile Completion</span>
          <span className="font-extrabold text-amber-400">92% Complete</span>
        </div>
        <div className="w-full h-3 rounded-full bg-slate-800 overflow-hidden">
          <div className="h-full bg-gradient-to-r from-amber-500 to-orange-400 w-[92%] rounded-full" />
        </div>
        <p className="text-[11px] text-slate-300">Upload your state electrical certification to reach 100% verified status.</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 sm:p-8 space-y-6 shadow-xl">
        <h3 className="text-lg font-bold text-slate-900 dark:text-white">Pending Job Approvals</h3>
            ...
            <div key={b.id} className="py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 text-[10px] font-bold">New Request</span>
                </div>
                <p className="text-xs text-slate-500">{b.scheduledDate} ({b.scheduledTime}) • Address: {b.address}</p>
              </div>
              <div className="flex items-center gap-3">
